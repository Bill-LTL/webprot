...
char failover_core(const char *time) {
    ...
    // 存活檢查
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "ping -6 -c 3 %s > /dev/null 2>&1", ip6.sever);
    ...
    if (is_down != 0) {  //0爲ping通，非0爲ping不通
        ...
        if (last_state == 0) {
            ...
            status=update_dns_api(time, ip6.host_ip);
        ...
        if (last_state == 0) {
            ...
            printf("%s [恢復] 服務器 %s 已上線！正在將 DNS 切換回原服務器...\n", time, ip6.sever);
            
            // 呼叫 API 更新 DNS 為原服務器 IP
            status=update_dns_api(time, ip6.sever);
...
char update_dns_api(const char *time, const char *new_ip) {
    ...
    printf("%s [DNS] 準備將記錄更新為: %s", time, new_ip);
    ...
    snprintf(cmd, sizeof(cmd),
        ...
        "--data '{\"type\":\"AAAA\",\"name\":\"%s\",\"content\":\"%s\",\"ttl\":60,\"proxied\":false}'",
        cfg.zoneID,   // 區域 ID
        cfg.recordID, // 記錄 ID
        cfg.api,      // API 令牌
        cfg.domain,   // 域名 
